module.exports = (db) => {
  ...
  const getTaskersByService = service_id => {
    ...
    const query = {
      text: `
      SELECT users.id, first_name, last_name, phone, email, address, coordinates, photo_url, summary, vehicle, hourly_rate, user_rating, reviews, availability
      FROM availabilities JOIN
        (
          SELECT users.id, first_name, last_name, phone, email, address, coordinates, photo_url, summary, vehicle, hourly_rate, AVG(user_rating) AS user_rating, COUNT(user_rating) AS reviews
          FROM users
  ...
  const getBestReviews = (limit) => {
    const query = {
      text: ` 
            SELECT execution_date, service_id, reviewer, tasker_id, user_comment, user_rating
            FROM (
                  SELECT
